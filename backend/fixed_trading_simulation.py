...
class OBPlusSimulator:
    ...
    def load_obplus_strategies(self):
        """Cargar estrategias OBPlus de forex_strategies_master (incluyendo momentum_continuacion)"""
        try:
            ...
            if result.data:
                ...
                print(f"📊 Fuente: forex_strategies_master (incluyendo momentum_continuacion)")
    ...
    def check_obplus_pattern(self, strategy, fragments, current_idx):
        ...
        try:
            ...
            # Estrategias adicionales
                ...
                # Si las 3 primeras velas son iguales, momentum continúa en siguiente fragmento
                if current_idx < len(fragments) - 1:
                    first_three = current_fragment['colors'][:3]
                    if len(set(first_three)) == 1:  # Las 3 primeras iguales
                        momentum_color = first_three[0]
                        return True, momentum_color
    ...
    def simulate_strategy(self, strategy, days_back=30, mg_level='MG0'):
        """Simular una estrategia OBPlus específica con fragmentos no solapados"""
        ...
        # Crear fragmentos de 5 velas NO solapados (como trading real)
        fragments = self.create_non_overlapping_fragments(candles)
        if len(fragments) < 5:
            print(f"❌ Fragmentos insuficientes: {len(fragments)}")
            return None
        
        print(f"📊 Trading real simulado: {len(fragments)} oportunidades de análisis")
        ...
        # Analizar cada fragmento de 5 velas (como en trading real)
        for i in range(len(fragments) - 1):  # -1 porque algunas estrategias necesitan siguiente fragmento
            ...
            if pattern_match and predicted_direction:
                ...
                if actual_outcome:
                    ...
                    simulation_log.append({
                        ...
                        'fragment_number': fragments[i]['fragment_number'],
        ...
        if simulation_log:
            ...
            print(f"  💼 Fragmentos analizados: {len(fragments)}")
            print(f"  📊 Operaciones ejecutadas: {total_operations}")
            print(f"  ⏱️ Operaciones por día: {total_operations / days_back:.1f}")
            ...
            results = {
                ...
                'operations_per_day': total_operations / days_back,
    ...
    def compare_mg_levels(self, strategy, days_back=30):
        ...
        for mg_level in ['MG0', 'MG1', 'MG2']:
            ...
            if result:
                ...
                print(f"  Operaciones/día: {result['operations_per_day']:.1f}")
    ...
    def run_comprehensive_simulation(self, top_strategies=10, days_back=30):
        ...
        print(f"📋 Metodología: Fragmentos NO solapados (trading real)")
        ...
        print(f"Enfoque: Fragmentos no solapados (trading real)")
        
        return all_results

def main():
    print("🎯 SISTEMA DE SIMULACIÓN OBPLUS - VERSIÓN CORREGIDA")
    print("Análisis de riesgo y efectividad con Martingala")
    print("Fragmentos NO solapados - Trading real simulado")
